...
public:
    ...
    enum class AvoidanceMode { On, Off };
     ...
     */
    bool checkIntersection(const Eigen::Vector2f& start, const Eigen::Vector2f& end, float safety_margin,
     ...
     */
    Eigen::Vector2f getAvoidanceTarget(const Eigen::Vector2f& robot_pos, const Eigen::Vector2f& target_pos,
                                       float safety_margin, float clearance, float robot_heading_rad,
                                       int recursion_depth = 0) const;

    /**
    * @brief Get an artificial potential field target vector using the manager's configured
    * parameters (`pf_ka`, `pf_kr`, `pf_influence_radius`).
    *
    * @param robot_pos Current robot position.
    * @param target_pos Desired goal position.
    * @param robot_heading_rad Current robot heading in radians.
    ...
    */
    Eigen::Vector2f getPotentialFieldTarget(const Eigen::Vector2f& robot_pos, const Eigen::Vector2f& target_pos,
                                            float robot_heading_rad) const;

    /**
     * @brief Set the avoidance mode.
     */
    ...
    /**
     * @brief Set safety margin and clearance used by avoidance routines.
     */
    void setAvoidanceParams(float safetyMargin, float clearance);

    /**
     * @brief Set potential field parameters.
     */
